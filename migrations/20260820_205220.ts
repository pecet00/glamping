...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_configuration_requests_status" AS ENUM('new', 'contacted', 'offer-sent', 'closed');
  CREATE TYPE "public"."enum_contact_requests_project" AS ENUM('single', 'park', 'hotel', 'other');
  CREATE TYPE "public"."enum_contact_requests_status" AS ENUM('new', 'contacted', 'closed');
  ...
  CREATE TABLE "configuration_requests" (
  	...
  	"status" "enum_configuration_requests_status" DEFAULT 'new' NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "contact_requests" (
  	...
  	"email" varchar NOT NULL,
  	"phone" varchar,
  	"project" "enum_contact_requests_project",
  	"message" varchar,
  	"contact_consent" boolean DEFAULT false NOT NULL,
  ...
  CREATE TABLE "payload_locked_documents_rels" (
  	...
  	"users_id" integer,
  	"extras_id" varchar,
  	"configuration_requests_id" integer,
  ...
  ALTER TABLE "payload_locked_documents_rels" ADD CONSTRAINT "payload_locked_documents_rels_configuration_requests_fk" FOREIGN KEY ("configuration_requests_id") REFERENCES "public"."configuration_requests"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "configuration_requests_updated_at_idx" ON "configuration_requests" USING btree ("updated_at");
  CREATE INDEX "configuration_requests_created_at_idx" ON "configuration_requests" USING btree ("created_at");
  ...
  CREATE INDEX "payload_locked_documents_rels_configuration_requests_id_idx" ON "payload_locked_documents_rels" USING btree ("configuration_requests_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "configuration_requests" CASCADE;
  ...
  DROP TYPE "public"."enum_configuration_requests_status";
  DROP TYPE "public"."enum_contact_requests_project";
